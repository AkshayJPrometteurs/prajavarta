...
export default function NewsListingScreen() {
    ...
    if (user?.role === 'ADMIN') {
        tabs = [
            ...
            { label: 'Author News', value: 'REPORTER' },
            { label: 'Pending Author News', value: 'PENDING_REPORTER' }
        ];
    } else {
    ...
    const handleApprove = async (item) => {
        try {
            ...
            toast.error(error.response?.data?.error || 'Failed to approve news')
        }
    }
    ...
    const handleReject = async (item) => {
        try {
            ...
            toast.error(error.response?.data?.error || 'Failed to reject news')
        }
    }
    ...
    const handleToggleStatus = async (item) => {
        try {
            ...
            const response = await axiosInstance.patch(endpoint, {
                ids: [item.id],
                action: item.ownerType === 'PENDING_REPORTER' ? 'approve' : undefined,
                isActive: item.ownerType === 'PENDING_REPORTER' ? true : !item.isActive
            })

            if (response.data.success) {
                toast.success(item.ownerType === 'PENDING_REPORTER' ? 'News approved successfully' : `News ${item.isActive ? 'disabled' : 'enabled'} successfully`)
                fetchNews()
            }
        } catch (error) {
                ...
                <div className="space-y-6 border-b border-base-200 p-5">
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                            {state.news.length > 0 && (
                                <ExportToExcel
                                    filename={`news_export_${Date.now()}.csv`}
                                    fetchUrl={`${user?.role === 'ADMIN' ? '/api/admin/news' : '/api/author/news'}?${new URLSearchParams({ ...(debouncedSearchTerm && { search: debouncedSearchTerm }), ...(state.categoryId !== 'all' && { categoryId: state.categoryId }), ownerType: state.activeTab, limit: '100000' })}`}
                                    headers={[{ label: 'ID', key: 'id' }, { label: 'Title', key: 'title' }, { label: 'Slug', key: 'slug' }, { label: 'Categories', key: 'categoryIds' }, { label: 'CreatedAt', key: 'createdAt' }, { label: 'IsActive', key: 'isActive' }]}
                            ...
                            >
                                ...
                                {state.activeTab === 'PENDING_REPORTER' ? (
                                    <>
                                        ...
                                        <div className="absolute bottom-0 left-0 right-0 p-3 pt-10">
                                            ...
                                            <div className="mb-2">
                                                <div className="flex flex-wrap items-center gap-2 text-xs font-medium text-white/80">
                                                    <span>{new Date(item.createdAt).toLocaleString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
                                                    {item.ownerType === 'PENDING_REPORTER' ? (
                                                        <span className="badge badge-error badge-sm">Pending</span>
                                                    ) : item.ownerType === 'REPORTER' ? (
                                                        <span className="badge badge-info badge-sm">Reporter</span>
                                                    ) : (
                                                ...
                                                <div className="flex items-center gap-2">
                                                    {item.ownerType === 'PENDING_REPORTER' ? (
                                                        ...
                                                        <input
                                                            ...
                                                            style={{
                                                                backgroundColor: item.isActive ? '#e11d48' : 'rgba(255,255,255,0.4)',
