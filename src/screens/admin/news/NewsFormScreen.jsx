...
export default function NewsFormScreen({ mode = 'add', newsId }) {
    ...
    useEffect(() => {
        if (mode === 'add' && user?.role === 'AUTHOR') {
            setFormValues((prev) => ({
                ...prev,
                ownerType: 'PENDING_REPORTER',
                isActive: false,
                authorId: user.id ? String(user.id) : prev.authorId
            }))
        }
    }, [mode, user])
                    ...
                    <Formik
                        ...
                        onSubmit={async (values) => {
                            try {
                                ...
                                const authorCreate = mode !== 'edit' && user?.role === 'AUTHOR'
                                const response = mode === 'edit'
                                    ? await axiosInstance.put(endpoint, { ...payload, id: newsId })
                                    : await axiosInstance.post(endpoint, {
                                        ...payload,
                                        ownerType: authorCreate ? 'PENDING_REPORTER' : payload.ownerType,
                                        isActive: authorCreate ? false : payload.isActive,
                                        authorId: authorCreate ? Number(user.id) : payload.authorId
                                    })
                                ...
                                if (response.data.success) {
                                    ...
                                    const route = user?.role === 'ADMIN' ? '/admin/news' : '/author/news'
                                    router.push(route)
                                }
                            } catch (error) {
                                ...
                                <Form className="space-y-4 px-6 pb-6 pt-3">
                                    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                                        ...
                                        {user?.role === 'ADMIN' ? (
                                            ...
                                            <Field type="hidden" name="authorId" />
                                        )}
